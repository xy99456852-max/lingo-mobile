...
const exerciseTotal=()=>SENTENCES.length+WORDS.length*4;
function home(){const pct=Math.round(learned.size/WORDS.length*100);return `<section class="hero"><span class="eyebrow">每天进步一点点</span><h1>把英语，练成你的<br>日常能力。</h1><p>无需登录，打开就学。学习记录只保存在你的设备上。</p></section><section class="mega-stats"><div><b>${massiveReady?WORDS.length.toLocaleString():'3,200+'}</b><span>分级词汇</span></div><div><b>${massiveReady?exerciseTotal().toLocaleString()+'+':'10,000+'}</b><span>动态练习</span></div><div><b>∞</b><span>无限模式</span></div></section><section class="progress-card"><div class="row"><span>已掌握词汇</span><b>${learned.size} / ${WORDS.length}</b></div><div class="meter"><i style="width:${pct}%"></i></div></section><section class="grid"><button class="feature" data-view="words"><span class="icon">Aa</span><h3>海量词库</h3><p>${massiveReady?WORDS.length.toLocaleString():'3,200+'} 个中英词条，搜索、分类、持续加载</p></button><button class="feature" data-view="sentences"><span class="icon">∞</span><h3>无限练习</h3><p>${massiveReady?exerciseTotal().toLocaleString()+'+':'10,000+'} 道题，随机生成、即时解析</p></button><button class="feature" data-view="grammar"><span class="icon">文</span><h3>语法手册</h3><p>从 A1 到 C1 的核心语法结构</p></button><button class="feature" data-view="translate"><span class="icon">↔</span><h3>快捷翻译</h3><p>常用短语离线互译，随时复制</p></button></section><p class="source-note">扩展词库来源：<a href="https://github.com/KyleBing/english-vocabulary" target="_blank" rel="noopener">KyleBing/english-vocabulary</a>，用于英语学习。</p>`}
...
fetch('./vocab.tsv').then(r=>{if(!r.ok)throw new Error('vocabulary unavailable');return r.text()}).then(text=>{let seen=new Set(WORDS.map(w=>w[0].toLowerCase()));for(const line of text.split(/\r?\n/)){let tab=line.indexOf('\t');if(tab<1)continue;let word=line.slice(0,tab).trim().toLowerCase(),raw=line.slice(tab+1).trim();if(!/^[a-z][a-z '-]{1,30}$/.test(word)||seen.has(word)||!raw)continue;seen.add(word);raw=raw.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');let type=(raw.match(/^(n\.|v\.|adj\.|adv\.|prep\.|conj\.|pron\.|num\.|int\.)/)||['常用词'])[0];WORDS.push([word,'扩展词库',type,raw,`I am learning how to use the word “${word}”.`,`我正在学习如何使用单词“${word}”。`,'初中'])}massiveReady=true;if(view==='home'||view==='words'||view==='sentences')render()}).catch(()=>{massiveReady=false});
